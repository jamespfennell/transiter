...
func MapScheduledServiceIDToPkInSystem(ctx context.Context, queries db.Querier, systemPk int64, scheduledServiceIDs ...[]string) (map[string]int64, error) {
	...
	if len(scheduledServiceIDs) > 0 {
		queryScheduledServiceIDs = scheduledServiceIDs[0]
		if len(queryScheduledServiceIDs) == 0 {
			return result, nil
		}
	}

	rows, err := queries.MapScheduledServiceIDToPkInSystem(ctx, db.MapScheduledServiceIDToPkInSystemParams{
		SystemPk:                   systemPk,
		FilterByScheduledServiceID: len(scheduledServiceIDs) > 0,
		ScheduledServiceIds:        queryScheduledServiceIDs,
	})
	if err != nil {
		return nil, err
	}
...
func MapShapeIDToPkInSystem(ctx context.Context, queries db.Querier, systemPk int64, shapeIDs ...[]string) (map[string]int64, error) {
	result := map[string]int64{}
	var queryShapeIDs []string
	if len(shapeIDs) > 0 {
		queryShapeIDs = shapeIDs[0]
		if len(queryShapeIDs) == 0 {
			return result, nil
		}
	}

	rows, err := queries.MapShapeIDToPkInSystem(ctx, db.MapShapeIDToPkInSystemParams{
		SystemPk:        systemPk,
		FilterByShapeID: len(shapeIDs) > 0,
		ShapeIds:        queryShapeIDs,
	})
	if err != nil {
		return nil, err
	}

...
type StopTree struct {
	pkToNode map[int32]*StopTreeNode
	base     *StopTreeNode
}
...
func NewStopTree(ctx context.Context, querier tdb.Querier, basePk int32) (*StopTree, error) {
	...
	for _, stop := range allStops {
		if stop.ParentStopPk.Valid {
			parentPk := stop.ParentStopPk.Int32
...
func (tree *StopTree) StationPks() []int32 {
	var result []int32
...
func (tree *StopTree) DescendentPks() []int32 {
	pksToVisit := []int32{tree.base.Stop.Pk}
	var result []int32
...
func (tree *StopTree) VisitDFS(visitFunc func(node *StopTreeNode)) {
	...
	seenPks := map[int32]bool{}
	seenPks[tree.base.Stop.Pk] = true
	visitedOncePks := map[int32]bool{}
	for len(stack) > 0 {
...
func (tree *StopTree) Get(stopPk int32) *tdb.Stop {

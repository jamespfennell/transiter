...
class Route(Base):
    ...
    regular_service_pattern_pk = Column(
        Integer, ForeignKey("service_pattern.pk", name="james_f_1")
    )
    regular_service_pattern = relationship(
        "ServicePattern", post_update=True, foreign_keys=[regular_service_pattern_pk]
    )
    default_service_pattern_pk = Column(
        Integer, ForeignKey("service_pattern.pk", name="james_f_2")
    )
    default_service_pattern = relationship(
        "ServicePattern", post_update=True, foreign_keys=[default_service_pattern_pk]
    )
    dynamic_service_pattern_pk = Column(
        Integer, ForeignKey("service_pattern.pk", name="james_f_3")
    )

...
class TestTripUpdater(testutil.TestCase(tripupdater)):
    ...
    def setUp(self):
        ...
        self.trip_1.stop_events = [
            models.StopTimeUpdate(stop_pk=int(stop_id), stop_id=stop_id, stop_sequence=i)
    ...
    def test_sync_trips(self, _sync_trips_in_route):
        ...
        tripupdater.sync_trips(self.system, [self.trip_1, self.trip_2], [self.ROUTE_1_ID])

        _sync_trips_in_route.assert_called_once_with(
            self.route_1, [self.trip_1], stop_id_to_pk)
        self.stopdam.get_id_to_pk_map_in_system. \
            assert_called_once_with(self.SYSTEM_ID, set(self.STOP_IDS))
        self.servicemapmanager. \
            calculate_realtime_service_map_for_route. \
            assert_called_once_with(self.route_1)
    ...
    def test_sync_trips_all_routes(self, _sync_trips_in_route):
        ...
        self.servicemapmanager. \
            calculate_realtime_service_map_for_route. \
            assert_has_calls(
                [mock.call(self.route_1), mock.call(self.route_2)],
                any_order=True
            )
    ...
    def test_sync_trips_in_route__new_trip(self):
        ...
        result = tripupdater._sync_trips_in_route(self.route_1, [self.trip_1], stop_id_to_stop_pk)
    ...
    def test_sync_trips_in_route__updated_trip_same_map(self):
        ...
        result = tripupdater._sync_trips_in_route(self.route_1, [feed_trip], stop_id_to_stop_pk)
    ...
    def test_sync_trips_in_route__updated_trip_new_map(self):
        ...
        stop_id_to_stop_pk = {
            stop_id: int(stop_id) for stop_id in set(self.STOP_IDS).union(self.STOP_IDS_2)}
        feed_trip = models.Trip(id=self.TRIP_ID)
        feed_trip.stop_events = [
            models.StopTimeUpdate(stop_id=self.STOP_IDS[1], stop_sequence=1),
